...
function drawGraph() {
    ...
    //console.log('graph.legendConfig.xOffset',graph.legendConfig.xOffset);
    ...
    //console.log(graph.svg.append('g').selectAll('.link').data(graph.force.links()).enter());

    graph.line = graph.svg.append('g').selectAll('.link')
        .data(graph.force.links())
      .enter().append('line')
      //.style('stroke', 'red');   //jophy
      .attr('class', 'link');
      ...
      .enter().append('g')
        ...
        .on('mouseover', function(d) {
            if (!selected.obj) {
                ...
                highlightObject(d);                   //mouseover highlight
            }
        })
    ...
    graph.node.each(function(d) {
        var node  = d3.select(this),
            ...
            lines = wrap(d.name),                     //node name defind
            ddy   = 1.1,
            dy    = -ddy * lines.length / 2 + .5;

        lines.forEach(function(line) {
            var text = node.append('text')            //node text
                .text(line)
                .attr('dy', dy + 'em');
            dy += ddy;
        });
